...
function iterator( k, lambda, options ) {
	...
	if ( !isPositiveNumber( k ) ) {
		throw new TypeError( format( '0qZ71', k ) );
	}
	if ( !isPositiveNumber( lambda ) ) {
		throw new TypeError( format( '0qZ72', lambda ) );
	}
	if ( arguments.length > 2 ) {
		if ( !isObject( options ) ) {
			throw new TypeError( format( '0qZ2V', options ) );
		}
		opts = assign( {}, options );
		if ( hasOwnProp( opts, 'iter' ) ) {
			if ( !isNonNegativeInteger( opts.iter ) ) {
				throw new TypeError( format( '0qZ2t', 'iter', opts.iter ) );
			}
		} else {
			opts.iter = MAX_VALUE;
		}
