...
function iterator( k, lambda, options ) {
	...
	if ( !isPositiveNumber( k ) ) {
		throw new TypeError( format( '0qZ71,NQ', k ) );
	}
	if ( !isPositiveNumber( lambda ) ) {
		throw new TypeError( format( '0qZ72,Go', lambda ) );
	}
	if ( arguments.length > 2 ) {
		if ( !isObject( options ) ) {
			throw new TypeError( format( '0qZ2V,FD', options ) );
		}
		opts = assign( {}, options );
		if ( hasOwnProp( opts, 'iter' ) ) {
			if ( !isNonNegativeInteger( opts.iter ) ) {
				throw new TypeError( format( '0qZ2t,G9', 'iter', opts.iter ) );
			}
		} else {
			opts.iter = MAX_VALUE;
		}
