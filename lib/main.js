...
function iterator( k, lambda, options ) {
	...
	if ( !isPositiveNumber( k ) ) {
		throw new TypeError( format( '0QB7P', k ) );
	}
	if ( !isPositiveNumber( lambda ) ) {
		throw new TypeError( format( '0QB7T', lambda ) );
	}
	if ( arguments.length > 2 ) {
		if ( !isObject( options ) ) {
			throw new TypeError( format( '0QB2h', options ) );
		}
		opts = assign( {}, options );
		if ( hasOwnProp( opts, 'iter' ) ) {
			if ( !isNonNegativeInteger( opts.iter ) ) {
				throw new TypeError( format( '0QB35', 'iter', opts.iter ) );
			}
		} else {
			opts.iter = MAX_VALUE;
		}
